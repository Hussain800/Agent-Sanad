...
def next_actions(fired_rules: list[str]) -> list[dict]:
    ...
    Case-aware logic: HARD-01 + CAP-01 together means no financial headroom,
    not missing hardship evidence. Only show hardship action when it's an
    evidence gap (unverified hardship), not a capacity constraint.
    """
    rule_set = set(fired_rules)
    actions: list[dict] = []

    # HARD-01 without CAP-01 = genuine hardship evidence gap → show action.
    # HARD-01 with CAP-01 = financial constraint → no beneficiary action.
    # HARD-01 with HARD-02 is a verified hardship, not a repair case.
    hard01_is_capacity = "HARD-01" in rule_set and "CAP-01" in rule_set
    hard01_is_verified = "HARD-01" in rule_set and "HARD-02" in rule_set

    if "DOC-01" in rule_set:
        actions.append({
    ...
    if "HARD-01" in rule_set and not hard01_is_capacity and not hard01_is_verified:
        actions.append({
