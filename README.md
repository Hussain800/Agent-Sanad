...
[![Tests](https://img.shields.io/badge/tests-431%20passing-1f7a4d?style=flat-square&logo=pytest&logoColor=white)](#-quality--testing)
...
flowchart TB
    ...
    subgraph API["FastAPI Service · 190 routes · uniform error envelope · JSON logs"]
...
**431 tests across 45+ files**, all passing. Run them:

```powershell
$env:PYTHONPATH="."
python -B -m pytest tests\ -q -p no:cacheprovider      # → 431 passed
```
...
agent-sanad/
├── backend/
│   ├── app.py                 # FastAPI app — 190 routes, error envelope, structured logs
...
| 6 | **Evaluation &amp; observability** | 94.6% historical benchmark; append-only audit; 431 tests; structured logs |
